...
class Language(nn.Module):
    ...
    def generate_hidden_head(self,heads,max_len=50,random=False):
        ...
        for i in range(len(heads)):
            ...
            cn=initialize((1,1,self.len_hidden)).cuda()
            for k in range(2):
                for j in range(max_len - 1 + k):
                    word = torch.LongTensor([word]).cuda()
                    word = self.embedding(word).view(1, 1, -1)
                    # print(word)
                    output, (hn, cn) = self.gate(word, (hn, cn))
                    output = self.fc(output)
                    choice = output.topk(6)[1][0][0]
                    # print(choice)
                    for word in choice:
                        # print(word.item())
                        if self.num_to_word[word.item()] == '。' or self.num_to_word[word.item()] == '，' \
                     		or self.num_to_word[word.item()] == '？' or self.num_to_word[word.item()] == '<end>'\
                        	or self.num_to_word[word.item()] == '！':
                           continue
                        else:
                            sentence.append(self.num_to_word[word.item()])
                            word = word.item()
                            break
                if k == 0:
                   sentence.append('，')
                   word = self.word_to_num['，']
                else:
                   sentence.append('。')
                   word = ['。']
            poem.append(sentence)
